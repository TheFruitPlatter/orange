 ...
 * with a timeout parameter. This executor removes and returns a single member with the minimum score from the sorted set,
 ...
 * such as task queues where lower scores represent higher priorities (e.g., timestamps for FIFO processing) and 
 * processing cannot wait indefinitely.
 *
 * @author Liang.Zhong
 * @since 1.0.0
 * @see <a href="https://orange.langwuyue.com/redis/advanced/zset">Orange Redis ZSet Documentation</a>
 */
public class OrangePopMemberByMinScoreTimeoutExecutor extends OrangeGetOneWithScoresAbstractExecutor {
	...
	/**
	 * Constructs a new executor for popping the lowest-scored member from a Redis ZSet with timeout support.
	 *
	 ...
	 */
	public OrangePopMemberByMinScoreTimeoutExecutor(OrangeRedisZSetOperations operations,OrangeRedisExecutorIdGenerator idGenerator) {
		super(idGenerator);
	 ...
	 * then calls the operations.popMinScore method with appropriate timeout settings.
	 *
	 ...
	 * 
	 * @return a list containing the PopMemberByMinScoreTimeout annotation class that this executor can process
	 */
	@Override
	protected List<Class<? extends Annotation>> getSupportedAnnotationClasses() {
		return OrangeCollectionUtils.asList(PopMembers.class,MinScore.class,Timeout.class);
	}
