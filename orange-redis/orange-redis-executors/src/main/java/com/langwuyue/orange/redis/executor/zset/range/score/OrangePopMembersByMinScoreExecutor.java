...
/**
 * Executor implementation for popping (removing and returning) multiple members with the lowest scores from a Redis ZSet.
 * This executor removes and returns a specified number of members with the minimum scores from the sorted set.
 * 
 * This is particularly useful in scenarios where you need to process items in order of priority,
 * such as task queues where lower scores represent higher priorities or earlier timestamps.
 *
 * @author Liang.Zhong
 * @since 1.0.0
 */
public class OrangePopMembersByMinScoreExecutor extends OrangeGetWithScoresAbstractExecutor {
	...
	/**
	 * Constructs a new executor for popping multiple lowest-scored members from a Redis ZSet.
	 *
	 ...
	 */
	public OrangePopMembersByMinScoreExecutor(OrangeRedisZSetOperations operations,OrangeRedisExecutorIdGenerator idGenerator) {
		super(idGenerator);
		this.operations = operations;
	}

	/**
	 * Executes the Redis ZSet pop operation to remove and return multiple members with the lowest scores.
	 * 
	 * This method casts the context to OrangeRedisCountContext to access the count parameter,
	 * then calls the operations.popMinScore method with the specified count to retrieve multiple members.
	 *
	...
	protected List<Class<? extends Annotation>> getSupportedAnnotationClasses() {
		return OrangeCollectionUtils.asList(PopMembers.class,MinScore.class,Count.class);
	}
