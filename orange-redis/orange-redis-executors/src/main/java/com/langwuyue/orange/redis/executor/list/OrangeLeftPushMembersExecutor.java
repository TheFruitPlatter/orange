...
/**
 * Executor implementation for pushing multiple elements to the left side of a Redis list.
 * 
 * <p>This executor extends {@link OrangeAddMembersAbstractExecutor} to provide
 * functionality for adding multiple elements to the beginning (left side)
 * of a Redis list. It supports the {@link Left}, {@link AddMembers}, {@link Multiple},
 * and {@link ContinueOnFailure} annotations.
 * 
 * <p>The executor adds elements to the list in reverse order, so that the rightmost
 * argument in the list of values becomes the leftmost element in the list.
 ...
 */
public class OrangeLeftPushMembersExecutor extends OrangeAddMembersAbstractExecutor {
	...
	/**
	 * Constructs a new OrangeLeftPushMembersExecutor with the specified operations, ID generator, and logger.
	 *
	 ...
	 */
	public OrangeLeftPushMembersExecutor(OrangeRedisListOperations operations,OrangeRedisExecutorIdGenerator idGenerator,OrangeRedisLogger logger) {
	...
	protected List<Class<? extends Annotation>> getSupportedAnnotationClasses() {
		return OrangeCollectionUtils.asList(Left.class,AddMembers.class,Multiple.class,ContinueOnFailure.class);
	}
	...
	/**
	 * Performs the left push operation for multiple values at once.
	 * 
	 ...
	 * then uses the {@code leftPush} operation with an array of values to push all elements
	...
	protected Long doAdd(OrangeRedisContext ctx) throws Exception {
		...
		return this.operations.leftPush(context.getRedisKey().getValue(), context.getValueType(), context.toArray());
	}

	/**
	 * Performs the left push operation for a single value.
	 * 
	 ...
	 * It pushes a single value to the left side of the Redis list.
	 *
	 * @param ctx the execution context containing the Redis key
	 * @param value the single value to push to the left of the list
	...
	protected Long doAdd(OrangeRedisContext ctx, Object value) throws Exception {
		return this.operations.leftPush(ctx.getRedisKey().getValue(), ctx.getValueType(), value);
	}
