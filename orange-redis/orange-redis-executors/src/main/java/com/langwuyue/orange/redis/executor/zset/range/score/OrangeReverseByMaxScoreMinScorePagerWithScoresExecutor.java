...
/**
 * Executor implementation for retrieving paginated members with their scores from a Redis ZSet in reverse order based on a score range.
 * 
 * This executor retrieves members whose scores fall within the specified range (defined by MaxScore and MinScore),
 * and returns them in reverse order (from highest to lowest score) along with their scores, with pagination support using a Pager object.
 * This is particularly useful for implementing detailed paginated leaderboards, time-based feeds with relevance scores,
 * or any scenario requiring paginated access to sorted data with scores in reverse order.
 * 
 * The executor supports pagination through the Pager annotation, which allows for more flexible pagination options
 * compared to simple offset/count pagination, and includes the scores of each member in the result.
 *
 * @author Liang.Zhong
 * @since 1.0.0
 * @see <a href="https://orange.langwuyue.com/redis/advanced/zset">Orange Redis ZSet Documentation</a>
 */
public class OrangeReverseByMaxScoreMinScorePagerWithScoresExecutor extends OrangeGetWithScoresAbstractExecutor {
	...
	/**
	 * Constructs a new executor for retrieving paginated members with their scores from a Redis ZSet in reverse order based on a score range.
	 *
	 ...
	 */
	public OrangeReverseByMaxScoreMinScorePagerWithScoresExecutor(OrangeRedisZSetOperations operations,OrangeRedisExecutorIdGenerator idGenerator) {
		super(idGenerator);
	 ...
	 * then calls the operations.reverseRangeByScoreWithScores method to retrieve the specified page of members with their scores
	 ...
	 * @return a collection of members with their scores from the sorted set that fall within the specified score range and page, in reverse order
	 ...
	 * 
	 * @return a list containing GetMembersWithScores, MaxScore, MinScore, and Pager annotation classes
	 *         that this executor can process for retrieving paginated members with their scores within a score range
	 *         in reverse order
	 */
	@Override
	protected List<Class<? extends Annotation>> getSupportedAnnotationClasses() {
		return OrangeCollectionUtils.asList(GetMembers.class,MaxScore.class,MinScore.class,WithScores.class,com.langwuyue.orange.redis.annotation.zset.Pager.class,Reverse.class);
	}
