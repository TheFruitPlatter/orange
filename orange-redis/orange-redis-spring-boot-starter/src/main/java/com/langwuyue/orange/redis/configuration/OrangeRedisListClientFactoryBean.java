...
import com.langwuyue.orange.redis.mapping.OrangeRedisListExecutorIdGenerator;
import com.langwuyue.orange.redis.mapping.OrangeRedisListExecutorsMapping;
...
public class OrangeRedisListClientFactoryBean extends OrangeRedisClientAbstractFactoryBean {
	...
	protected OrangeRedisExecutorsMapping getExecutorsMapping() {
		...
		EXECUTORS_MAPPING = new OrangeRedisListExecutorsMapping(
			operations, 
			new OrangeRedisListExecutorIdGenerator(),
			getListeners(),
			scriptOperations,
			getMultipleListener(),
			getLogger()
	...
	@Override
	protected Class<? extends OrangeRedisCircuitBreaker> getCircuitBreaker​​Class(){
		this.client = this.getClientDefinitionClass().getAnnotation(OrangeRedisListClient.class);
